...
plot_fig2_beta <- function(physeq, plot_title="") {
  ...
  ord_mat <-
    ordinate(physeq,
             ...
             weighted = T)
  dist_mat1 <-
    phyloseq::distance(physeq %>% subset_samples(group %in% c("CTR","CA")), method = "unifrac", weighted = T)
  ...
  dist_mat2 <-
    phyloseq::distance(physeq %>% subset_samples(group %in% c("CA","VOR")), method = "unifrac", weighted = T)

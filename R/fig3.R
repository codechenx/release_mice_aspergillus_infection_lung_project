...
plot_fig3_beta <- function(physeq, plot_title="") {
  distance = "unifrac"
  weighted = T
...
plot_correlation_between_omics_chord <- function(cor_tbl) {
  ...
  links <- cor_tbl %>%
    ...
    mutate(X=str_replace_all(X, "microbiome", "tissue microbiome"), Y=str_replace_all(Y, "microbiome", "tissue microbiome")) %>%
...
plot__modima_result <- function(pbs_modiam_tbl,wt_modima_tbl, ppta_modima_tbl) {
  ...
  merge_tbl <- bind_rows(pbs_modiam_tbl, wt_modima_tbl, ppta_modima_tbl) %>%
    ...
    mutate(path = str_replace_all(path, "micro", "tissue microbiome")) %>%
    mutate(path = str_replace_all(path, "meta", "metabolome")) %>%
    filter(path %in%  c("Gut tissue microbiome -> Plasma metabolome -> Lung tissue microbiome",
                        "Gut tissue microbiome -> Plasma metabolome -> Lung metabolome",
                        "Lung tissue microbiome -> Plasma metabolome -> Gut tissue microbiome",
                        "Lung metabolome -> Plasma metabolome -> Gut tissue microbiome")) %>%
    mutate(path = factor(path, levels= c(
      "Lung metabolome -> Plasma metabolome -> Gut tissue microbiome",
      "Lung tissue microbiome -> Plasma metabolome -> Gut tissue microbiome",
      "Gut tissue microbiome -> Plasma metabolome -> Lung tissue microbiome",
      "Gut tissue microbiome -> Plasma metabolome -> Lung metabolome"))) %>%
